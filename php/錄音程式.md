...
- 產生 `.md` 文字稿
- 更新 `index.json` 索引
- 錄音檔移動到 NAS backups/ 備份
- 刪除本機快取，避免重複處理

### �發方式
- **指令**: `#ttsphone` 或直接傳送錄音檔

### 本機目錄結構
```
C:\Users\linyi\.openclaw\workspace\
├── temp/
│   ├── recording.m4a              # 臨時錄音檔（處理後刪除）
...
result = model.transcribe('temp/audio.wav', language='Chinese')
...
TEMP_DIR = 'C:\\Users\\linyi\\.openclaw\\workspace\\temp'
...
def process_recording(audio_path, original_filename):
    # 1. Whisper 轉文字
    subprocess.run(['python', 'transcribe.py'], cwd=TEMP_DIR)
    
    # 2. 讀取轉錄結果
    with open(f'{DATA_DIR}\\transcript.json', 'r', encoding='utf-8') as f:
...
"""
    ...
    index['files'].append({
        ...
        'backup_path': f'backups/{backup_name}',  # 新增欄位
  ...
  "files": [
    {
      ...
      "backup_path": "backups/file_4_20260206_1150.m4a",
      "transcript_path": "transcripts/1707187200_abc123.md",
      "date": "2026-02-06 11:50:00",
      "duration": 771,
      "size": 201469,
      "status": "done"
    }
...
| `filename` | 原始檔名 |
| `path` | 錄音檔相對路徑 (保留供相容性) |
| `backup_path` | **新增**：備份檔相對路徑 (backups/{original}_{timestamp}.m4a) |
...
sudo mkdir -p /web/voice-sync/backups        # 新增：錄音備份區
